/**
 * =========================================================
 * HISTORY_TRANSAKSI.JS - COMPLETE v2.0
 * Features:
 * - PDF Generation with jsPDF
 * - Optimized QR Code (compact signature)
 * - QR Scan verification (mobile-ready)
 * - Remove manual verification (useless)
 * =========================================================
 */

console.log('📋 [HISTORY_TRANSAKSI v2.0] Loading...');
...
function init_history_transaksi() {
    console.log('🚀 Init History Transaksi v2.0');
    loadNotaHistory();
}
...
function renderNotaRow(t, index) {
    ...
    return `
        <tr>
            ...
            <td>
                <button class="btn primary btn-sm" onclick='viewNotaDetail(${JSON.stringify(t).replace(/'/g, "&#39;")})'>
                    👁️ View
                </button>
                <button class="btn success btn-sm" onclick='generateNotaPDF(${JSON.stringify(t).replace(/'/g, "&#39;")})'>
                    📄 PDF
                </button>
            </td>
        </tr>
    ...
    const detailHTML = `
        <div style="text-align: left;">
            ...
            <div style="margin-top: 20px; text-align: center;">
                <button class="btn success" onclick='generateNotaPDF(${JSON.stringify(transaction).replace(/'/g, "&#39;")})'>
                    📄 Download PDF Nota
                </button>
            </div>
        </div>
...
console.log('✅ [HISTORY_TRANSAKSI v2.0] Module loaded');