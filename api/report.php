<?php
// FILE: api/report.php
// Fungsi: Menyediakan data statistik, monitoring, dan riwayat transaksi untuk Owner.
session_start();
...
if (!isset($_SESSION['user']) || $_SESSION['user']['role'] !== 'owner') {
    api_response(false, "Akses ditolak. Hanya Owner yang diizinkan melihat laporan.", null, 403);
}
...
if ($method === 'GET') {
    ...
    try {
        if ($action === 'summary') {
            ...
            ";
            $stmt = $pdo->query($sql);
            $history = $stmt->fetchAll(PDODO::FETCH_ASSOC);
