function showUserForm(user = null) {
    ...
    showMessageModal(
        ...
        `Yakin ingin menonaktifkan akun <b>${username}</b>? Akun tidak dapat login lagi.`,
        true, // isConfirm = true
        async () => { // Callback saat user klik OK
            showLoadingModal('Menonaktifkan user...');
            try {
                // Menggunakan fetch dengan method DELETE
                const response = await fetch(`api/admin_users.php?id=${id}`, {
                    method: 'DELETE'
                });
                const data = await response.json();
                ...
                if (data.success) {
                    showMessageModal('✅ Sukses', data.message, false);
                    loadPage('admin_users'); // Muat ulang daftar setelah sukses
                } else {
                    showMessageModal('❌ Gagal', data.message, false);
                }
            } catch (error) {
...
// Fungsi utama untuk memuat dan merender tabel daftar pengguna
// Fungsi ini harus terdaftar di objek window agar dapat dipanggil dari app.js
window.init_admin_users = async function() {
    const listDiv = document.getElementById('userListPanel');
    if (!listDiv) return; // Keluar jika elemen panel tidak ditemukan
    ...
    try {
        ...
        // Mulai membangun tabel HTML
        let tableHtml = '<div class="card"><h3>Daftar Pengguna SWIMS</h3><table class="table"><thead><tr>';
        tableHtml += '<th>ID</th><th>Username</th><th>Role</th><th>Status</th><th>Dibuat</th><th>Aksi</th></tr></thead><tbody>';
        
        // Iterasi melalui data pengguna dan membuat baris tabel
        data.data.forEach(user => {
            const statusBadge = user.is_active == 1 
                ? '<span class="badge badge-success">Aktif</span>' 
                : '<span class="badge badge-danger">Non-aktif</span>';
            
            // Tombol "Non-aktifkan" hanya muncul jika user aktif dan BUKAN user yang sedang login
            const deactivateButton = (user.is_active == 1 && user.username !== currentUser()?.username) ? 
                `<button class="btn danger btn-sm" onclick="deactivateUser(${user.id}, '${user.username}')">Non-aktifkan</button>` 
                : '';

            // Perlu melakukan JSON.stringify(user) agar objek bisa dilewatkan ke fungsi JS di onclick
            tableHtml += `
                <tr>
                    <td>${user.id}</td>
                    <td>${user.username}</td>
        ...
        tableHtml += '</tbody></table></div>';
        listDiv.innerHTML = tableHtml; // Render tabel ke DOM
